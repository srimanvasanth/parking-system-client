...
const Parking = () => {
    ...
    const openParkingModal = (data) => {
        console.log(data);
        setSelectedSlot(data?.slotNbr)
        if(!data?.vehicleId?.plateNbr){
            setOpenParking(true);
        }        
    }
    ...
    const parkVehicle = (plateNbr) => {
        postMethod({
            url: `${apiurl}/parkVehicle`,
            data: {plateNbr, vehicleType, slotNbr: selectedSlot},
        })
    } 
    ...
    return (
        <>
            ...
            <div className='d-flex justify-content-center'>
                <h2 className='text-center text-white mt-2 fs-3'>{vehicleType === 2 ? "TWO WHEELER PARKING" : "FOUR WHEELER PARKING"}</h2>
            </div>
            ...
            <Container className="w-50 d-flex flex-column" style={{minHeight: "70vh"}}>
                <div className="flex-grow-1">
                    <Row className="row-cols-4">
                        {slicedData?.map((data) => (
                            <Col key={data?._id}>
                                <Card className={`${data?.vehicleId?.plateNbr ? "btn bg-secondary" : "btn btn-success"} d-flex justify-content-center align-items-center mt-5 pt-3 mb-3`} onClick={() => openParkingModal(data)}>
                                    {vehicleType === 2 ? <MdTwoWheeler size={60}/>  : <FaCar size={60}/>}
                                    <CardBody>
                                        <CardTitle className="mx-auto">
                                            Slot No: {data?.slotNbr}
                                        </CardTitle>
                                        {<CardText className="text-center">{data?.vehicleId?.plateNbr ? (data?.vehicleId?.plateNbr).toUpperCase() : "Available"}</CardText>}
                                    </CardBody>
                                </Card>                            
                            </Col>
                        ))}
                ...
                <Pagination className="justify-content-end align-items-end mt-3">
                    {pageArr.map((_, i) => (
            ...
            {openParking && <ParkUnparkModal
                open={openParking}
                close={() => closePopup()}
                title={"Park"}
                apiCall={(plateNbr) => parkVehicle(plateNbr)}
            />}
